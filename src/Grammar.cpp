...
Non_terminal* Grammar:: get_non_terminal(int id){
...
Non_terminal* Grammar:: get_non_terminal(string name){
  list<Non_terminal>::iterator it;
  for(it=this->non_terminals.begin();it!=this->non_terminals.end();it++){
    if((*it).name == name){
        return &(*it);
...
rule* Grammar::get_rule(int id){
 ...
 int Grammar:: get_terminal_id(string name){
...
string Grammar::get_terminal(int id){
  list<string>::iterator it;
  int i=0;
  for(it=this->terminals.begin();it!=this->terminals.end();it++){
    if(i == id){
 ...
 bool Grammar::termContain(string name){
     bool found = false;
     list<string>::iterator it;
     for(it=this->terminals.begin();it!=this->terminals.end();it++){
     if((*it) == name){
